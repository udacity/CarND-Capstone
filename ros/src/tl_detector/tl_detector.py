...
LIGHT_MINIMUM_DETECTION_DISTANCE = 250 #defines min distance to look for traffic lights ahead
LIGHT_CLASSIFIER_ON = False #True uses classifier, False uses simulator data to debug other nodes
...
class TLDetector(object):
	...
	#We use realtime classification using the camera
	if LIGHT_CLASSIFIER_ON:
	...
	line_wp_idx = None
		...
		for i, light in enumerate(self.lights):
			...
			if d > 0.0 and d < diff and d < LIGHT_MINIMUM_DETECTION_DISTANCE: 
				...
				#to save useless CPU calculations. We are allowed to know light 				#positions, so classifier is used to color detection :)
