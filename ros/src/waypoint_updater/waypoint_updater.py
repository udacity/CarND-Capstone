...
class WaypointUpdater(object):
    ...
    def __init__(self):
        ...
        # Member variables
        self.base_waypoints            = None # Waypoint list as it is published once
        ...
        self.duration_1_max    = 0;
        self.duration_2_max    = 0;
        self.duration_max      = 0;
        self.dt_max            = 0;
        self.temp              = 0;
        ...
        # Dictionary with stop line position for each traffic light
        self.stop_line_positions       = config['stop_line_positions']
        # Traffic light positions and states
        self.lights                    = None
    ...
    def loop(self):
        '''
        Updates and publishes final waypoints with a rate of 50Hz.
        '''
        ...
        while not rospy.is_shutdown():
            ...
            # Get next waypoint ahead of the car for current psition. Starting
            # from second iteration, search only next to the last offset.
            if self.prev_base_offset != 0:
            ...
            for i in range(len(lookahead_waypoints)):
                original_velocities.append(
                    lookahead_waypoints[i].twist.twist.linear.x)
            # Accelerate from first to last waypoint using specified
            # acceleration and also reduce the number of waypoints to cover only
            # a specified distance ahead of the car.
            lookahead_waypoints = self.accelerate(lookahead_waypoints, start)
            # Decelerate, or accelerate from stop waypoint backwards using
            ...
            for i in range(len(lookahead_waypoints)):
                self.prev_waypoint_velocities.append(
                    lookahead_waypoints[i].twist.twist.linear.x)
