...
class WaypointUpdater(object):
	...
	for p in pc2.read_points(msg,skip_nans=True,field_names=('x','y','z')):
		...
		obstacle_list.append([prev_idx,distance])
	self.obstacles=np.array(obstacle_list)
	#print(self.obstacles)	
    #calulate the distance from a point to a line which contains two point
    def distance2line(self,p1,p2,p3):
	xa=p1[0]
	ya=p1[1]
	xb=p2[0]
	yb=p2[1]
	xc=p3[0]
	yc=p3[1]
	h=((yb-yc)*xa-(xb-xc)*ya+yc*(xb-xc)-xc*(yb-yc))/math.sqrt((xb-xc)*(xb-xc)+(yb-yc)*(yb-yc))
	return h
