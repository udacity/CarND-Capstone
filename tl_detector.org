...
#+BEGIN_SRC python :noweb tangle :tangle
  ...
  for light_cursor in range(len(self.stop_line_positions)):
      ...
      if base_waypoint_search_cursor < self.base_waypoints_num:
          dist_shorttest = dl(self.base_waypoints[base_waypoint_search_cursor].pose.pose.position,
                              self.stop_line_positiotns[light_cursor])
          ...
          for i in range(base_waypoint_search_cursor+1, self.base_waypoints_num):
              dist = dl(self.base_waypoints[base_waypoint_search_cursor+1+i].pose.pose.position,
                        self.stop_line_positiotns[light_cursor])
              if dist < d_shortest:
                  d_shortest = dist
