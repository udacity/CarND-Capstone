...
Data = []
TRAINING_DATA_DIR = 'raw_img/'

number_of_all_class = [0,0,0]

with open('sim_raw_images.csv') as csvfile:
    ...
    for line in reader:
        ...
        if light_state == 0 and next_light_wp < 300:
            repeat = 1
        elif light_state == 1 and next_light_wp < 300:
            repeat = 3
            is_red_light = 0
        elif light_state == 2 and next_light_wp < 300:
            repeat = 2
        else:
            repeat = 0
...
def generator(samples, aug_data = False, batch_size=32):
...
model.save('tl_classifier_simulator_raw.h5')
